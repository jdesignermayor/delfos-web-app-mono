  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Inmobiliarias</h1>
          <p className="mt-1 text-sm text-muted">Gestiona las inmobiliarias del sistema</p>
        </div>
        <Link href="/dashboard/inmobiliarias/new" className={buttonVariants({ variant: "primary", size: "sm" })}>
          Nueva inmobiliaria
      ...
      <Card className="p-5">
        {inmobiliarias.length === 0 ? (
          <p className="text-center text-muted">No hay inmobiliarias registradas</p>
        ) : (
                ...
                {inmobiliarias.map((item) => (
                  <tr key={item.id} className="hover:bg-surface-secondary/60">
                    ...
                    <td className="py-3 pl-4 text-right">
                      <Link href={`/dashboard/inmobiliarias/${item.id}`} className="text-sm text-accent hover:underline">
                        Editar
                      </Link>
                    </td>
                  </tr>
